...
| stop   | D    | 属性词 | 该物体不能被跨越（不允许一个东西移动到stop物体位置上） |
...
具体而言，Masou会给你一张描述关卡图的n\*m矩阵，每个位置都有一个代号（见上表），对应游戏中的词条/物体，保证初始图同一位置至多出现一个东西。另外，所有东西都只能待在n\*m的范围内。
...
对于执行语法部分，所有的语法在同一时刻**并行执行**。（如果 X is X 和 X is Y 同时满足，则 X is X 优先级更高）
...
共T行，每行输出YES或NO，表示程序是否在执行操作序列的某一步通关。
